...
class PaymentResultFactoryTest extends TestCase
{
    public function test_payment_result_factory_create_defaults()
    {
        ...
        // Assert
        $this->assertSame($processorTxnId, $result->getProcessorTxnId());
        $this->assertSame(round((float) 3400, 2), $result->getTxnAmount());
        $this->assertSame($txnId, $result->getTxnId());
        $this->assertSame('22890775623', $result->getTxnPayeerId());
        $this->assertSame($at, $result->getTxnTime());
        $this->assertSame('T-REF-830535', $result->getTxnReference());
    }
